...
//Height evaluation using forward derivative
//float dHdxy_fwd
...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input) {
	...
	output.Normal = mul(normalize(float4(input.Normal, 1)), World);
...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0{
	...
	// using bumpmapping
	//float4 height = tex2D(BumpMapSampler, input.TextureCoord);
	//float3 perturbedNormal = PerturbNormal(input.Position, input.Normal, height.r * 10);
	
	// get normal from map
	float3 normal = tex2D(BumpMapSampler, input.TextureCoord).rgb * 2.0 - 1.0;

	// texture color
	float4 texDiffuse = tex2D(MaterialTextureSampler, input.TextureCoord);

	// specular map
	float specularValue = tex2D(SpecularMapSampler, input.TextureCoord).r * 2;
	...
	// position normal
	//float4 normal = float4(perturbedNormal, 1);  // do I need to localize? (multiply by worldviwtranspose?)
	...
	float4 ambient = texDiffuse * 0.5;

	// calculate diffuse light values
	float4 diffuse = texDiffuse * 0.7 * diff;
